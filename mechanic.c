...
int main(void) {
    ...
    garage = (tGarage*) realloc(garage, sizeof (tGarage) * 2);
    ...
    garage[1].number = 2;
    garage[1].speciality = 2;

    currentNrOfGarages = 2;

    mechanic = (tMechanic*) realloc(mechanic, sizeof (tMechanic) * 2);
    mechanic[0].name = GetMechanicName();
    mechanic[0].nr = 1;
    mechanic[0].nrOfShifts = 1;
    mechanic[0].dateOfBirth.day = 1;
    mechanic[0].dateOfBirth.month = 1;
    mechanic[0].dateOfBirth.year = 1;
    mechanic[0].speciality = 1;

    mechanic = GetMechanicShift(mechanic, currentNrOfMechanics, 0,
            garage, currentNrOfGarages);


    mechanic[1].name = GetMechanicName();
    mechanic[1].nr = 2;
    mechanic[1].nrOfShifts = 1;
    mechanic[1].dateOfBirth.day = 1;
    mechanic[1].dateOfBirth.month = 1;
    mechanic[1].dateOfBirth.year = 1;
    mechanic[0].speciality = 0;


    mechanic = GetMechanicShift(mechanic, currentNrOfMechanics, 1,
            garage, currentNrOfGarages);

    currentNrOfMechanics += 2;
    //mechanic = InsertMechanic(mechanic, &currentNrOfMechanics, garage, currentNrOfGarages);
    //mechanic = InsertMechanic(mechanic, &currentNrOfMechanics, garage, currentNrOfGarages);
    do {
        ShowAllMechanic(mechanic, currentNrOfMechanics);
       mechanic =  EditMechanic(mechanic, currentNrOfMechanics, garage, currentNrOfGarages);
        printf("\n FIM");
    } while (true);
    //ShowAllMechanic(mechanic, currentNrOfMechanics);
    return 0;
